...
const Register = () => {
  ...
  const Auths = async (platform: string) => {
    setLoading(true);
    await SocialAuth(platform, "register", setUser, navigate);
    setLoading(false);
  };
  ...
  const submitUser = async (): Promise<void> => {
    ...
    setLoading(true);
    await axios
      .post("http://localhost:4000/api/v1/user/register", {name: name, email: email, password: password})
      .then((response: any) => {
    ...
    <main className="  px-5 md:px-20 my-8  ">
      ...
      <div className=" mt-8 grid  grid-cols-1 md:grid-cols-2   ">
        <form className=" text-left">
          ...
          <input ref={nameref} type="text" name="name" placeholder="Please Enter your name" className=" outline-none bg-transparent dark:text-white px-3 py-3 w-full  border-[0.1px] mt-2  " />
